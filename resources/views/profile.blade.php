...
@section('content')
    ...
    <div id="consultas-container" class="col-md-12">
        ...
        @if(request('search'))
            <div class="mb-3">
                <a href="{{ route('consultas') }}" class="badge
bg-danger text-decoration-none" style="font-size: 1rem;">
                    Limpar Pesquisa
                </a>
            </div>
        @endif
...
0px 4px 6px rgba(0, 0, 0, 0.1); max-width: 280px;">
                        <img
src="https://imovbsb.com/wp-content/uploads/2017/10/consultas-medicas.webp"
...
{{$consulta->horario}}</p>
                            ...
                            <a href="{{route('exibir_consulta',
$consulta->id)}}" class="btn" style="background-color: #002244; color:
white; padding: 10px 20px; border-radius: 5px; margin-top:
10px;">Visualizar</a>
                            @if (Auth::user()->id == $consulta->user_id)
                                <a href="{{route('editar_consulta',
$consulta->id)}}" class="btn" style="background-color: #6c757d; color:
white; padding: 10px 20px; border-radius: 5px; margin-top:
10px;">Editar</a>
                            @endif
                        </div>
